      ...
      <ChartCard title="유종별 매출 순위" subtitle="단위: 만원">
        <ResponsiveContainer width="100%" height={Math.max(300, data.length * 36)}>
          <BarChart data={data} layout="vertical" margin={{ left: 20 }}>
            ...
            <YAxis type="category" dataKey="name" tick={{ fill: "#d1d5db", fontSize: 11 }} axisLine={false} width={90} />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left py-2 px-2 text-[11px] font-semibold text-gray-500 uppercase">유종</th>
                ...
                <th className="text-right py-2 px-2 text-[11px] font-semibold text-gray-500 uppercase">점유율</th>
              </tr>
            </thead>
            <tbody>
              {data.map((r) => (
                <tr key={r.name} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="py-2 px-2 text-xs text-emerald-400 text-right">{r.share.toFixed(1)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
